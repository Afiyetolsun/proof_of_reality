...
/**
 * Inline 3D preview for gallery cards. Default state is a static
 * gradient (zero WebGL contexts, fast page load); clicking the play
 * affordance mounts the appropriate viewer:
 *
 *   GLB / glTF → <model-viewer> with auto-rotate, no controls
 *   USDZ       → three-usdz-loader via UsdzCanvas (Pixar USD WASM)
 *
 ...
 *
 * The CardThumbnail behind us renders the gradient. We mount over it
 * absolute-positioned so the play interaction is one continuous
 * element with the visual.
 */
import { useEffect, useRef, useState } from "react";
import Script from "next/script";
import { UsdzCanvas } from "@/app/[name]/UsdzCanvas";

type Format = "glb" | "usdz" | "unknown";

interface Props {
  url: string;
  /** Used for prefetching the HEAD probe — we run it on viewport-enter
   *  so the play button can render the right viewer instantly. */
  prefetch?: boolean;
}

export function CardScene({ url, prefetch = true }: Props) {
  ...
  return (
    <div ref={containerRef} className="absolute inset-0">
      ...
      {playing && (format === "usdz" || format === "unknown") && (
        <>
          <UsdzCanvas url={url} />
          <StopButton onClick={() => setPlaying(false)} />
        </>
...
async function probeFormat(url: string): Promise<Format> {
  try {
    ...
    return "usdz"; // assume USDZ (current iOS pipeline) when probe is ambiguous
  } catch {
    return "unknown";
  }
}
